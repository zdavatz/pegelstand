#!/usr/bin/env python3
"""Build the CJK subset font for the `teaching` binary (ZH/JA PDFs).
...
This script collects every character actually used in texts_zh.rs/texts_ja.rs,
...
must not be committed). Re-run after changing the ZH/JA texts, then:

    CJK_FONT=~/.config/pegelstand/fonts/ArialUnicode-CJK-subset.ttf \
        cargo run --release --bin teaching -- ZH JA
...
chars = set()
for name in ("texts_zh.rs", "texts_ja.rs"):
