...
- `login-pair.mjs` — **experimental/unreliable** login via 8-char pairing code instead of QR (for remote/headless). Failed 4× on 2026-07-10 against Baileys `7.0.0-rc13` (phone: "Gerät konnte nicht hinzugefügt werden"; socket stuck `registered:false`; close code 408 "attempts ended"), while QR login worked on the first scan immediately after. **Prefer `login-qr.mjs`**; keep this only in case a stable Baileys release fixes the pairing-code handshake.
